...
class ToInterpolatedGrid(EEGTransform):
    ...
    def __init__(self,
        ...
        self.channel_location_dict = channel_location_dict
        self.max_electrode_num = len(channel_location_dict)
    ...
    def apply(self, eeg: np.ndarray, **kwargs) -> np.ndarray:
        if eeg.shape[0] > self.max_electrode_num:
            raise ValueError(
                f'Number of electrodes in eeg ({eeg.shape[0]}) is larger than the number of electrodes in channel_location_dict ({self.max_electrode_num})'
            )
